...
void testFunction( Mat &srcImg, Mat &dstImg, std::vector<std::vector<Point>> &contours_filtered )
{
    Mat grayImg, edgeImg, cannyImg;

    cvtColor( srcImg, grayImg, CV_BGR2GRAY );
    //cv::blur( grayImg, grayImg, Size( 3, 3 ) );
    //threshold( grayImg, edgeImg, 90, 255, CV_THRESH_OTSU );

    adaptiveThreshold( grayImg, edgeImg, 255, CV_ADAPTIVE_THRESH_MEAN_C, CV_THRESH_BINARY, 15, 3 );

    Mat kernel = getStructuringElement( MORPH_RECT, Size( 3, 3 ) );
    cv::morphologyEx( edgeImg, edgeImg, MORPH_OPEN, kernel );
    cv::morphologyEx( edgeImg, edgeImg, MORPH_CLOSE, kernel );

    Canny( edgeImg, cannyImg, 1, 254, 3 );

    //对得到的两条轮廓统计横座标值，确定轮廓位置

    int line_l = 0, line_l_max = 0;
    int line_r = cannyImg.cols, line_r_max = 0;
    for (int i = 5; i < 12; i++)
    {
        float avgl = 0, avgr = 0;
        for (int j = 0;j < cannyImg.rows;j++)
        {
            avgl += (float) cannyImg.at<uchar>( j, i ) / cannyImg.rows;
            avgr += (float) cannyImg.at<uchar>( j, cannyImg.cols - i ) / cannyImg.rows;
        }
        //将canny左右座标作为最终座标确定
        ...
        {
            line_r = cannyImg.cols - i;
            line_r_max = avgr;
        }
    }

    //对对应边缘上的线的值进行取反操作
    for (int i = 0;i < cannyImg.rows - 1;i++)
    {
        cannyImg.at<uchar>( i, line_l ) = 255 - cannyImg.at<uchar>( i, line_l );

        cannyImg.at<uchar>( i, line_r ) = 255 - cannyImg.at<uchar>( i, line_r );
    }

    std::vector<std::vector<Point>> contours;
    std::vector<Vec4i> hierarchy;

    cv::findContours( cannyImg, contours, hierarchy, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_NONE, Point( 0, 0 ) );
    ...
    if (contours_filtered.size() != 0)
    {
        ...
        for (int i = 0; i < contours_filtered.size(); ++i)
        {
            ...
            rectangle( dstImg, box[i], Scalar( 0, 0, 255 ), 1 );
...
#define AREAG   0    //G
    ...
    Mat AreaBImg = imread( "Pic\\E\\area\\B\\B.png" );


    Mat dstBImg;
    std::vector<std::vector<Point>> contours_filtered;

    testFunction( AreaBImg, dstBImg, contours_filtered );

    //最终结论
    if (contours_filtered.size() != 0)
    {
        cout << "B区伤痕数量为：" << contours_filtered.size() << endl;
    }
    else
        cout << "B区没有伤痕!" << endl;
   ...
   // Two_PassNew( edgeEImg, labelImg );
    ...
    Mat AreaGImg = imread( "Pic\\E\\area\\G\\G4.png" );
    ...
    //将最后一个值强制定在末尾
    lines_filter[lines_filter.size()-1] ={ cannyGImg.cols-6,1,false } ;

    vector<Mat> GcellImg;
    GcutImage( AreaGImg, lines_filter, GcellImg );

    //G区分区成功
    cout << "G区  分区成功！" << endl;

    //for (int i = 0;i < GcellImg.size();i + 2)
    {
        //Mat src = GcellImg[0];
        //Mat dst;
    ...
    ard.drawLines( AreaGImg, lines_filter, dstGImg );
